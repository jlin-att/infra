...
def main():
    ap = argparse.ArgumentParser(
        description=(
            "Per site: max metrics + time (and optional entity/CPT); output CSV to stdout; "
    ...
    # For the new file, maxima may occur on different CPTs; capture that name if present.
    ap.add_argument("--entity-col", default="CPT name", help="Optional column for node/entity name (default: CPT name)")

    # Compute columns (present in your example)
    ...
    def print_alerts(metric_col: str, label: str, threshold: float, value_name: str):
        ...
        alerts = (
            ...
            .loc[base[metric_col] > threshold, cols]
