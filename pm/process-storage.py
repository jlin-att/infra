...
def main():
    ap = argparse.ArgumentParser(
        description=(
            "Per site: max metrics + time (and optional entity/storage node); output CSV to stdout; "
    ...
    # For this file, storage identity is typically in STR name
    ap.add_argument(
        "--entity-col",
        default="STR name",
        help="Optional column for storage/entity name (default: STR name)",
    )

    # Storage metric columns (as in your sample)
    ap.add_argument("--cpu-col", default="Storage CPU Utilization")
    ap.add_argument("--mem-col", default="Storage Memory In Use Ratio")
    ap.add_argument("--thr-col", default="Storage Data Throughput")
    ap.add_argument("--disk-col", default="Storage VFS Disk Usage")
    ...
    # Strip whitespace from headers in case the file has "colname " etc.
